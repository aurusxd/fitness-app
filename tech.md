...
- Названия упражнений из ответа ИИ сопоставляются с таблицей `exercises` по имени (нормализация: lowercase, trim); не найдено — создаётся новая запись в `exercises` через сервис, а не напрямую. **(v4)** У таких упражнений `muscleGroup = 'unspecified'` (в JSON-контракте группа мышц не передаётся); ручная категоризация — в стадии 4.
