...
<DialogPrimitive.Portal>
	...
	<DialogPrimitive.Content
		bind:ref
		class={cn(
			'fixed top-1/2 left-1/2 z-50 w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-[var(--radius-lg)] border border-border bg-card p-6 text-card-foreground shadow-xl outline-none',
			className
		)}
		{...restProps}
	>
