...
<div class="mx-auto flex max-w-2xl flex-col gap-6 px-6 py-6">
	<h1 class="font-display text-lg font-bold">Журнал тренировок</h1>

	<div class="flex gap-2.5">
		<Card class="flex-1">
			<CardContent class="p-4 text-center">
				<div class="font-display text-xl font-extrabold">{data.summary.exercisesLogged}</div>
				<div class="mt-0.5 text-[11px] font-semibold text-muted-foreground">Упражнений</div>
			</CardContent>
		</Card>
		<Card class="flex-1 border-primary bg-primary">
			<CardContent class="p-4 text-center">
				...
				<div class="mt-0.5 text-[11px] font-semibold text-primary-foreground/70">Подходов</div>
			</CardContent>
		</Card>
		<Card class="flex-1">
			<CardContent class="p-4 text-center">
				<div class="font-display text-xl font-extrabold">{data.summary.trainingDays}</div>
				<div class="mt-0.5 text-[11px] font-semibold text-muted-foreground">Дней</div>
			</CardContent>
		</Card>
	</div>
	...
	<Card>
		<CardContent class="p-4.5">
			<div class="mb-1 flex items-center justify-between">
				<span class="text-xs font-semibold text-muted-foreground">Подходы по дням</span>
			...
			<div class="font-display text-2xl font-extrabold">
				{data.summary.setsLogged}
				<span class="text-sm font-semibold text-muted-foreground">подходов</span>
			</div>
			...
			<svg
				...
				role="img"
				aria-label="Подходы по дням за последние 14 дней"
			>
				<polyline
	...
	{#if data.entries.length === 0}
		<p class="text-center text-sm text-muted-foreground">
			Пока ничего не отмечено. Открой программу и отметь упражнение, когда сделаешь его.
		</p>
	{/if}
					...
					<Card>
						<CardContent class="flex items-center justify-between gap-4 p-4">
							...
							<Badge variant="secondary">
								{entry.setsDone} × {entry.repsDone}{entry.weightKg ? ` · ${entry.weightKg} кг` : ''}
							</Badge>
						</CardContent>
					</Card>
				{/each}
